<?php
/**
  * @var \App\View\AppView $this
  */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Retaileremployees Employeerole'), ['action' => 'edit', $retaileremployeesEmployeerole->retailerEmployee_id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Retaileremployees Employeerole'), ['action' => 'delete', $retaileremployeesEmployeerole->retailerEmployee_id], ['confirm' => __('Are you sure you want to delete # {0}?', $retaileremployeesEmployeerole->retailerEmployee_id)]) ?> </li>
        <li><?= $this->Html->link(__('List Retaileremployees Employeeroles'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Retaileremployees Employeerole'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Retaileremployees'), ['controller' => 'Retaileremployees', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Retaileremployee'), ['controller' => 'Retaileremployees', 'action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Employeeroles'), ['controller' => 'Employeeroles', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Employeerole'), ['controller' => 'Employeeroles', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="retaileremployeesEmployeeroles view large-9 medium-8 columns content">
    <h3><?= h($retaileremployeesEmployeerole->retailerEmployee_id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Retaileremployee') ?></th>
            <td><?= $retaileremployeesEmployeerole->has('retaileremployee') ? $this->Html->link($retaileremployeesEmployeerole->retaileremployee->id, ['controller' => 'Retaileremployees', 'action' => 'view', $retaileremployeesEmployeerole->retaileremployee->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Employeerole') ?></th>
            <td><?= $retaileremployeesEmployeerole->has('employeerole') ? $this->Html->link($retaileremployeesEmployeerole->employeerole->id, ['controller' => 'Employeeroles', 'action' => 'view', $retaileremployeesEmployeerole->employeerole->id]) : '' ?></td>
        </tr>
    </table>
</div>
