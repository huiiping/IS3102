...
class SessionsActivityComponent extends Component
{
    ...
    public function startup(Event $event)
    {
        ...
        $user_id = $this->_session->read('Auth.Retailer_Employees.id');
        ...
        $record = compact('controller', 'action', 'params', 'expires', 'retailer_employee_id', 'user_agent', 'user_ip', 'full_url', 'modified');
     ...
     * @return array
     */
    public function getOnlineRetailerEmployees()
    {
        $sessions = TableRegistry::get('Sessions');
        ...
        $records = $sessions
            ->find('expires')
            ->contain([
                'Retailer_Employees' => function ($q) {
                    return $q->select([
                        'id',
                        'username'
                    ]);
                },
                'Retailer_Employees.Groups' => function ($q) {
                    return $q->select(['css', 'is_member']);
                },
            ])
            ->select(['Sessions.retailer_employee_id', 'Sessions.expires'])
            ->group('Sessions.retailer_employee_id')
            ->toArray();
    ...
    public function getOnlineStatus(User $user)
    {
        ...
        $online = $sessions
            ->find('expires')
            ->select(['Sessions.expires', 'Sessions.retailer_employee_id'])
            ->where([
                'Sessions.retailer_employee_id' => $user->id
            ])
            ->first();
    ...
    public function getOnlineSessionsForUser($user)
    {
        ...
        $sessions = $this->Sessions
            ->find('expires')
            ->where([
                'Sessions.retailer_employee_id' => $user
            ])
            ->toArray();
